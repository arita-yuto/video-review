...
※ サーバー上で作業していないときは、サーバーのホスト名と `compose.prod.yml` で公開しているポートに読み替えます  
※ 初回起動では数秒間「Database is preparing...」と出ます（自動で再試行します）  
※ セットアップ画面は未初期化のときだけ開けます（以降はルート URL からログイン画面に移ります）  
※ パスワードをリセットする画面はないので、管理者の認証情報は失くさないように保管します
...
連携と General の設定は、以前は `.env` に書いていました  
`.env` の値は、管理画面で保存するまでは今までどおり読まれます

管理画面で保存すると、その項目は保存した値が使われ、`.env` を書き換えても効かなくなります  
`.env` の該当する変数は `.example.env` の Deprecated の節にまとめてあります
